...
public class user_frame extends JFrame {
    ...
    private JPanel user_information_panel = null;
    private JPanel mainPanel = new JPanel();
    private List<JButton> allButtons = new ArrayList<>();
    private JLabel name_label=new JLabel(SaveUserStateTool.getUser().getName());
    ...
    private JPanel user_information_panel() {
        if (user_information_panel == null) {
            user_information_panel = new JPanel();
            user_information_panel.setLayout(null);
            user_information_panel.setBounds(120, 35, 100, 100);
            user_information_panel.setBackground(Color.BLACK);
            name_label.setForeground(Color.white);
            name_label.setFont(new Font("宋体", Font.BOLD, 16));
            name_label.setBounds(0, 0, 100, 20);
            JLabel role_label = new JLabel("[用户]");
            role_label.setForeground(Color.white);
            role_label.setFont(new Font("宋体", Font.BOLD, 16));
            role_label.setBounds(0, 70, 100, 20);
            user_information_panel.add(role_label);
            user_information_panel.add(name_label);
        }
        return user_information_panel;
    }
    ...
    private JPanel getAvatar_panel() {
        if (avatar_panel == null) {
            ...
            BufferedImage img = ImageUtils.loadAndResizeImage(admin_frame.class, SaveUserStateTool.getUser().getAvatarUrl(), 100, 100);
            if (img != null) {
     ...
     * 根据页面名称触发左侧菜单按钮的点击行为
     */
    private void triggerMenuButton(String pageName) {
        switch (pageName) {
            case "home":
                if (homeButton != null) homeButton.doClick();
                break;
            case "book_list":
                if (bookListButton != null) bookListButton.doClick();
                break;
            case "add_book":
                if (addBookButton != null) addBookButton.doClick();
                break;
            case "user_list":
                if (userListButton != null) userListButton.doClick();
                break;
            case "add_manage":
                if (addUserManageButton != null) addUserManageButton.doClick();
                break;
            case "borrow_book":
                if (borrowBookButton != null) borrowBookButton.doClick();
                break;
            case "return_book":
                if (returnBookButton != null) returnBookButton.doClick();
                break;
            case "info_page":
                if (infoPageButton != null) infoPageButton.doClick();
                break;
            case "edit_page":
                if (editPageButton != null) editPageButton.doClick();
                break;
            default:
                System.out.println("未知页面：" + pageName);
                break;
        }
    }
    ...
    private RoundedPanel infopanel() {
        ...
        shouye_panel.setMaximumSize(new Dimension(200, 30)); // 控制高度为30px
        shouye_panel.setMinimumSize(new Dimension(200, 30));
        shouye_panel.setPreferredSize(new Dimension(200, 30));
        shouye_panel.setLayout(new BoxLayout(shouye_panel, BoxLayout.X_AXIS)); // 推荐使用布局管理器
...
// 将按钮加入全局按钮列表中
    ...
    private void showhomepage() {
        ...
        cardLayout.show(mainPanel, "user_home");
    }

    private void return_book() {
        previousPage = currentPage;
        currentPage = "return_book";
        CardLayout cardLayout = (CardLayout) mainPanel.getLayout();
        cardLayout.show(mainPanel, "userreturn_book");
    }

    private JPanel getreturn_book() {
    ...
    private void borrow_book() {
        previousPage = currentPage;
        currentPage = "borrow_book";
        CardLayout cardLayout = (CardLayout) mainPanel.getLayout();
        cardLayout.show(mainPanel, "borrow_book");
    }

    private JPanel getborrow_book() {
        return new BorrowBookPanel();
    }
    ...
    private JPanel getedit_page() {
        return new Edituserpanel();
    }

    private void addUserManagePage() {
        previousPage = currentPage;
        currentPage = "add_manage";
        CardLayout cardLayout = (CardLayout) mainPanel.getLayout();
        cardLayout.show(mainPanel, "add_manage");
    }

    private JPanel getadd_manage() {
        return new AddUserManagePanel();
    }

    private void showBookListPage() {
        previousPage = currentPage;
        currentPage = "book_list";
        CardLayout cardLayout = (CardLayout) mainPanel.getLayout();
        cardLayout.show(mainPanel, "book_list");
    }

    private JPanel getbook_list() {
        return new BookListPanel();
    }
    ...
    private JPanel getUserListPage() {
    ...
    private JPanel getinfo_page() {
        return new adminInfoPanel();
    }
    ...
    private JPanel getnavpanel() {
        if (navpanel == null) {
            navpanel = new JPanel();
            navpanel.setLayout(null); // 回归绝对布局以保持原有结构
            navpanel.setBackground(Color.BLACK);
            navpanel.setPreferredSize(new Dimension(200, this.getHeight()));
            navpanel.setBounds(0, 0, 200, this.getHeight());

            // 创建 infopanel 并放入 JScrollPane
            RoundedPanel infoPanel = infopanel(); // 原有 infopanel
            ...
            // 获取并设置管理员信息面板位置
            JPanel adminInfo = user_information_panel();
    ...
    /**
     * 配置 JScrollPane 的垂直滚动条样式
     *
     * @param scrollPane 要配置的 JScrollPane
     */
    private void configureVerticalScrollBar(JScrollPane scrollPane) {
        ...
        verticalScrollBar.setUI(new BasicScrollBarUI() {
            ...
            protected void configureScrollBarColors() {
                this.thumbColor = Color.GRAY; // 滚动条滑块颜色
            }
            ...
            protected JButton createDecreaseButton(int orientation) {
                return new JButton() {
                    ...
                    public Dimension getPreferredSize() {
                        return new Dimension(0, 0); // 隐藏上方向按钮
                    }
                };
            }
            ...
            protected JButton createIncreaseButton(int orientation) {
                return new JButton() {
                    @Override
                    public Dimension getPreferredSize() {
                        return new Dimension(0, 0); // 隐藏下方向按钮
                    }
                };
            }
        });
    ...
    private JPanel getmainpanel() {
        ...
        configureVerticalScrollBar(scrollPane);
        configureVerticalScrollBar(scrollPane3);

        mainPanel.add(new home_panel(), "user_home");
        mainPanel.add(new Edituserpanel(), "edit_page");         // 编辑页面
        mainPanel.add(new BorrowBookPanel(), "borrow_book");     // 借阅图书
        String name = SaveUserStateTool.getUser().getName();
        mainPanel.add(new userreturnbook(name), "userreturn_book");//归还图书
        mainPanel.add(new userInfoPanel(), "info_page");         // 查看信息
    ...
    public void initialize() {
        ...
        SaveUserStateTool.addUserChangeListener(newUser -> {
            SwingUtilities.invokeLater(() -> {
                // 更新界面显示
                name_label.setText(newUser.getName());
    ...
    private void MOuselistener() {
        ...
        windowtitle.titleBar.addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseDragged(MouseEvent e) {
                Point location = user_frame.this.getLocation();
                user_frame.this.setLocation(location.x + e.getX() - mousex, location.y + e.getY() - mousey);
            }
        });
    }
